...
偏好表单、城市概览、按偏好生成安排、每日行动卡、地图与官方入口、模拟预订锁定、结构化晚出门/少走路重排预览及确认、雨天室内替换、恢复上一版本、预算参考、文字回顾、偏好记忆复用、本机保存、JSON/文字导出、全程打印。
...
覆盖预订保留与重新生成、无原地修改、时间顺序、日期与金额合法性、锁定与交通冲突、缓冲时段、URL 协议校验、记忆与兴趣、精确小数、人民币换算、每日账本汇总，以及受约束重排的预览隔离、锁定/交通不可移动、顺延/压缩/移除与步行替换。构建与 TypeScript 检查通过；尚未执行浏览器交互或 PNG 导出视觉测试。
